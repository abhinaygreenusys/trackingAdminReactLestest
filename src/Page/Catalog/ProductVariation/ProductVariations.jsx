...
const ProductVariations = () => {
  ...
  const [productUnits, setProductUnits] = useState([{ id: 1, name: 'Unit 1', attribute: 'Chest' }]);
  const [newUnit, setNewUnit] = useState({ name: '', attribute: 'SelectAttribute' });
    ...
    if (editUnit) {
      setProductUnits(prevUnits =>
        prevUnits.map(unit => unit.id === editUnit.id
          ? { ...unit, name: newUnit.name, attribute: newUnit.attribute }
          : unit
        )
      );
      setEditUnit(null);
      ...
      setProductUnits([...productUnits, { id: newId, name: newUnit.name, attribute: newUnit.attribute }]);
    }
  ...
  const handleEditClick = (unit) => {
    setEditUnit(unit);
    setNewUnit({ name: unit.name, attribute: unit.attribute });
